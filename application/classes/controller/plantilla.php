...
class Controller_plantilla extends Controller {
    ...
    public function action_word($id = '') {
        ...
        WHERE d.id='$id'";
            ...
            $data[] = array(
                ...
                'cargod' => utf8_decode($documento->cargo_destinatario),
                'via' => utf8_decode($documento->nombre_via),
                'cargovia' => utf8_decode($documento->cargo_via),
                'remitente' => utf8_decode($documento->nombre_remitente),
                'cargor' => utf8_decode($documento->cargo_remitente),
                'referencia' => utf8_decode($documento->referencia),
                'fecha' => utf8_decode(date::fecha($documento->fecha_creacion)),
                'fechas' => utf8_decode(date::fechasindia($documento->fecha_creacion)),
                'mosca' => $documento->mosca_remitente,
                'copia' => $documento->copias,
                'cites' => utf8_decode($documento->cite_original),
                'hr' => $documento->nur,
            ...
            $plantilla = $tipos->template;
            if ($documento->nombre_via != '') {
                $plantilla=$tipos->template_via;
            }
...
//$file_name = str_replace('.','_'.date('Y-m-d').'.',$template);
            $filext=explode(".", $plantilla);
            $file_name = 'muserpol' . time() .'.'.$filext[1];
// Output as a download file (some automatic fields are merged here)
    ...
    public function action_informe($id = '') {
        ...
        $data[] = array(
            //'i'=>$i,
            'destinatario' => utf8_encode($documento->nombre_destinatario),
            'cargod' => utf8_decode($documento->cargo_destinatario),
            'via' => utf8_decode($documento->nombre_via),
            'cargovia' => utf8_decode($documento->cargo_via),
            'remitente' => utf8_decode($documento->nombre_remitente),
            'cargor' => utf8_decode($documento->cargo_remitente),
            'referencia' => utf8_decode($documento->referencia),
            'fecha' => utf8_encode(date::fecha($documento->fecha_creacion)),
            'mosca' => $documento->mosca_remitente,
            'copia' => $documento->copias,
            'cites' => $documento->cite_original,
            'hr' => $documento->nur,
            'adjunto' => $documento->adjuntos,
            'nur' => $documento->nur,
            'oficina' => $oficina->sigla,
        ...
        $TBS->Plugin(TBS_INSTALL, OPENTBS_PLUGIN); // load OpenTBS plugin
//cargamos el template segun el tipo de documento
        $template = 'templates/informe.docx';
        $debug = 0;
        $suffix = '';
